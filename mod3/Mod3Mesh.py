...
class Mod3MeshPartHeader(CS.PyCStruct):
	fields = OrderedDict([
		...
		("vertexSub", "ulong"),
		# Offset to start of current kind vertex block within vertex array
		("vertexOffset", "ulong"),
		("blocktype", "ulong"),
		("faceOffset", "ulong"), # Offset to start of faces within face array
		("faceCount", "ulong"), # Count of faces
		# Running subtotal of spilled vertices of the same kind
		("vertexBase", "ulong"),
		("NULL_0", "ubyte"), # 0
		("boundingBoxCount", "ubyte"),
		("unknownIndex", "ushort"),
		("vertexSubMirror", "ushort"), # Copy of Vertex Sub
		# Current last vertex index on the subtotal (Subtotal + Count - 1) after current
		("vertexIndexSub", "ushort"),
...
class Mod3Mesh():
	...
	@staticmethod
	def weightFunctionSelector(x):
		return {0: Mod3Mesh.unifiedWeightFunction,
	...
	def decomposeVertices(self, vertices, splitWeights):
		...
		flat_vertices = [(vertex.position.x, vertex.position.y,
						vertex.position.z) for vertex in vertices]
...
class Mod3MeshCollection():
	...
	def updateCountsOffsets(self):
		...
		for mesh in self.Meshes:
			mesh.updateCounts()
			vCount, vBufferLen, prevBase, prevVertexSub, prevBlockSize = mesh.updateVertexOffsets(
				vCount, vBufferLen, prevBase, prevVertexSub, prevBlockSize)
			fCount += mesh.faceCount()
		self.vertexOffset = len(Mod3Mesh(0, 0).Header) * \
			len(self.Meshes) + len(self.BoundingBoxes)
		self.faceOffset = self.vertexOffset + vBufferLen
		currentFaceOffset = 0
		for mesh in self.Meshes:
			currentFaceOffset = mesh.updateFaceOffsets(
				self.faceOffset, currentFaceOffset)
...
class BoundingBox():
	def __init__(self, mod3bb):
		...
		self._metadata = {field: getattr(mod3bb, field)
							for field in Mod3BoundingBox.fields}
...
class Mod3BoundingBoxes(CS.PyCStruct):
	...
	def sceneProperties(self):
		properties = {"MeshProperty%05d:%s" % (ix, prop): val for ix, propertyFamily in enumerate(
			self.BoundingBoxes) for prop, val in propertyFamily.sceneProperties().items()}
